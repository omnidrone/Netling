...
namespace Netling.ConsoleClient
{
    public class ConsoleClient
    {
        ...
        public void Start()
        {
            ...
            if (count.HasValue)
                RunWithCount(url, _requestSequence, count.Value).Wait();
            else if (url != null)
                RunWithDuration(url, _requestSequence, threads, TimeSpan.FromSeconds(duration)).Wait();
            else
                ShowHelp();
        }
